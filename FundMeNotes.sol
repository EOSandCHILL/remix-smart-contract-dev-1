...
contract FundMe { 
    ...
    uint256 public minimumUSD = 50 * 1e18; // we need to match the standard 18 decimal places
    ...
    function fund() public payable {
        ...
        require(msg.value.getConversionRate() >= minimumUSD, "Didnt send enough ETH!!!"); // "msg" is a global keyword in sol. "msg.value" retrieves how much value someone is sending. require() will check if the right amount was sent. if the amount was not enough revert and send message "Didnt send enough ETH!!!". Revert undos any actions that happened before the requirement was not met and sends remaining gas back
